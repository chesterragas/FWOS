...
namespace ROHM_WOS.Areas.WorkOrderRequest.Models
{
    public class DataHelper
    {
        ...
        public String GetData(string field, string table, string condition, string asVar = "")
        {
            ...
            try
            {
                using (SqlConnection conn = new SqlConnection(ConfigurationManager.ConnectionStrings["STI_IDMS"].ConnectionString.ToString()))
                {
                    conn.Open();
        ...
        public DataTable UpdateColumnID(DataTable table, string ColumnIDName, int HeaderID)
        {
            foreach (DataRow row in table.Rows)
            {
                row[ColumnIDName] = HeaderID;
            }
            return table;
        }
        public DataTable UpdateDrawingNoID(DataTable table, int HeaderID)
        {
            foreach (DataRow row in table.Rows)
            {
                row["mDrawingNo_ID"] = HeaderID;
            }
            return table;
        }
